...
`POST /api/room-assignments` SHALL create a new active assignment. Body: `room_id` (required), `tenant_id` (required), `start_date` (required, date string), `notes` (optional). Validates: room must exist and be `available`; tenant must exist and have no active assignment. On success: creates assignment record AND sets room.status = 'occupied'. Returns 201 with created assignment.
