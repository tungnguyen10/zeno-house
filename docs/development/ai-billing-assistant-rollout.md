...
`NUXT_PUBLIC_AI_DEV_CHAT_ENABLED` only renders the dashboard UI. `NUXT_AI_CHAT_ENABLED` authorizes the server route. Both models go through OpenRouter, so this configuration does not need `NUXT_AI_GOOGLE_API_KEY`. `NUXT_AI_ALLOW_PAID_PRIMARY` is a private, explicit cost opt-in; when it is absent or false, production rejects a primary without `:free`. The fallback remains free-only in every mode.

## Release Gates

1. Run `npm run verify:ai-models` with the exact deployment environment loaded. It must find both model IDs and `tools` support, confirm the fallback has zero prompt/completion pricing, and reject non-zero primary pricing unless `NUXT_AI_ALLOW_PAID_PRIMARY=true`.
...
2. Temporarily configure an unavailable primary while keeping Gemma as fallback. Verify the request completes before streaming through Gemma and reports `fallbackUsed=true`. Restore the primary immediately.
...
7. Check OpenRouter usage reflects only the approved paid primary requests, fallback usage remains zero-cost, and logs/telemetry contain no prompt, message body, raw tool payload, or secret.
