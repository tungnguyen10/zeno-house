...
- [x] 1.1 Tạo migration `user_building_assignments`: table schema, unique constraint `(user_id, building_id)`, index trên `user_id`
- [x] 1.2 Thêm backfill SQL trong migration: cross-join tất cả managers × buildings, `can_delete_master_data = false`
- [x] 1.3 Cập nhật `app/types/database.types.ts` với type mới cho `user_building_assignments`

## 2. Permission Model (Spec 1 + Spec 2 foundation)

- [x] 2.1 Thêm `billing.corrections` vào `permissions.ts` — admin set và manager set
- [x] 2.2 Cập nhật `server/services/billing/invoices.ts`: `void`, `reissue`, `adjustment` check `billing.corrections` thay vì `billing.write`
- [x] 2.3 Thêm type declaration cho `event.context.__buildingScope` trong `H3EventContext` (tại `app/types/auth.ts` hoặc `server/utils/scope.ts`)

## 3. Scope Resolver & Assignment Repository (Spec 1 core)

- [x] 3.1 Tạo `server/repositories/assignments.ts`:
...
- [x] 3.2 Tạo `server/utils/scope.ts`:
...
- [x] 4.3 `server/services/tenants/index.ts`:
...
- [x] 4.4 `server/services/contracts/index.ts`:
  - `list`: filter `buildingIds` từ scope
  - `get`: `assertBuildingScope(mode: 'read')` theo `room.buildingId`
  - `create`: `assertBuildingScope(mode: 'write')` theo building của room
  - `update`, `remove`, `bulkAction`: `assertBuildingScope(mode: 'write')`
- [x] 4.5 `server/services/contract-occupants.ts`: `assertBuildingScope` via `contract → room → building`
- [x] 4.6 `server/services/contract-payments.ts`: `assertBuildingScope` via `contract → room → building`
- [x] 4.7 `server/services/contract-renewals.ts`: `assertBuildingScope` via `contract → room → building`
- [x] 4.8 `server/services/contract-services.ts`: `assertBuildingScope` via `contract.building_id` hoặc `building_id` trực tiếp
- [x] 4.9 `server/services/building-services/`: `assertBuildingScope` theo `building_id`
- [x] 4.10 `server/services/meter-readings/`: filter list theo `buildingIds`, `assertBuildingScope` cho detail/write
- [x] 4.11 `server/services/billing/periods.ts`:
  - `list`: filter `buildingIds`
  - `get`, draft, issue, close, unissue, overview: `assertBuildingScope`
- [x] 4.12 `server/services/billing/invoices.ts`: `assertBuildingScope` via `invoice → billing_period → building_id` (2-hop)
- [x] 4.13 `server/services/billing/payments.ts`: `assertBuildingScope` via `invoice → period → building`
- [x] 4.14 `server/services/billing/audit.ts`: `assertBuildingScope` theo `billing_period.building_id`
- [x] 4.15 `server/repositories/dashboard/index.ts`: accept `buildingIds?: string[]` parameter, inject `.in('building_id', buildingIds)` vào tất cả queries

## 5. Dashboard Service Scope (Spec 1)

- [x] 5.1 `server/services/dashboard/index.ts` (hoặc nơi gọi repository): resolve scope → pass `buildingIds` vào `DashboardRepository.getSummary`
- [x] 5.2 Verify global counts (tenants, contracts) cũng được filter đúng khi manager
...
- [x] 7.1 `server/api/assignments/index.get.ts` — list tất cả managers + assignments (admin-only)
- [x] 7.2 `server/api/assignments/index.post.ts` — tạo assignment (admin-only)
- [x] 7.3 `server/api/assignments/[id].patch.ts` — toggle `can_delete_master_data` (admin-only)
- [x] 7.4 `server/api/assignments/[id].delete.ts` — xoá assignment (admin-only)
- [x] 7.5 `server/api/assignments/buildings-without-manager.get.ts` — danh sách buildings không có manager (admin-only, dùng cho warning UI)
- [x] 7.6 Tạo validators `app/utils/validators/assignments.ts`: create schema, update schema

## 8. Settings UI (Spec 3 frontend)

- [x] 8.1 Thêm "Settings" nav item vào `app/utils/constants/navigation.ts` (chỉ hiển thị với `isAdmin`)
- [x] 8.2 Cập nhật `app/components/app/AppSidebar.vue`: conditional render Settings nav item khi `isAdmin`
- [x] 8.3 Tạo `app/pages/settings/managers.vue`:
...
- [x] 8.4 Tạo composable `app/composables/useManagerAssignments.ts`: fetch, assign, unassign, toggle
- [x] 8.5 Cập nhật `app/pages/buildings/[id]/settings.vue`: thêm section "Managers" — list assigned managers (read-only với manager, với link về `/settings/managers` cho admin)
