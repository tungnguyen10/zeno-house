# Spec: Manager Assignment UI

## Purpose

Admin-only UI at `/app/managers` for viewing all manager accounts and managing their building assignments and per-feature permissions via `building_managers`.
...
- **THEN** the same array is returned (building visibility for the dropdown is not restricted by building_managers; permission enforcement happens at the feature level)
