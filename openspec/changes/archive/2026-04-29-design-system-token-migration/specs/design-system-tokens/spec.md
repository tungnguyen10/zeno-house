...
Header SHALL có button để toggle dark mode. State được persist trong localStorage. Toggle MUST dùng `.dark` class trên `<html>` element.
...
Toàn bộ Vue components và layouts SHALL không chứa các class sau: `bg-gray-*`, `text-gray-*`, `border-gray-*`, `bg-white` (ngoại trừ khi white là semantic — e.g. trong dark zones), `dark:bg-*`, `dark:text-*`, `dark:border-*`.
