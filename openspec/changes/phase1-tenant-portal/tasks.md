...
- [ ] 1.2 Create `server/api/tenant/me/contract.get.ts` — return tenant's active contract
- [ ] 1.3 Create `server/api/tenant/me/profile.patch.ts` — update profiles + tenants tables atomically
- [ ] 1.4 Create `locales/vi/tenant-portal.json` + `locales/en/tenant-portal.json`
...
- [ ] 3.4 Create `app/components/features/tenant-portal/MaintenanceForm.vue` — title, description, priority select, photo upload (max 3)
...
- [ ] 4.2 Implement `app/pages/tenant/contract.vue` — uses `TenantPortalContractView` component
...
- [ ] 4.5 Implement `app/pages/tenant/profile.vue` — edit form + change password section
