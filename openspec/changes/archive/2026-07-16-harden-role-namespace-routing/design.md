...
Dedicated print-only routes remain inside `/dashboard` but MAY set `layout: false` so printed output excludes the internal sidebar and header. This is a presentation exception only; authentication and namespace enforcement still apply.
