...
5. **Render HTML and PDF from one server document model.** `InvoicePrintCard` is the content and hierarchy standard: a focused `InvoiceDocumentData` builder resolves its status, meter metadata, charge labels, current totals, and stored payment-profile asset paths. The escaped HTML renderer and PDFKit renderer consume that same shape; PDFKit recreates the A4 print hierarchy rather than executing Vue in the worker. QR/logo bytes are downloaded server-side only from snapshot paths; valid optional images are embedded in the email with Resend CID attachments and otherwise use the established fallback.
