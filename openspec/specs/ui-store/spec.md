...
Pinia store for shared UI state. Currently manages sidebar open/close state used across layout components without prop drilling.
...
- **THEN** `closeSidebar()` is called automatically via a watcher in `default.vue`
