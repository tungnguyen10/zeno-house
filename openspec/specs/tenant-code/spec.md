## Purpose
Defines the unique code for each tenant in format `{nameInitials}-{year}-{seq}`, used as a non-PII URL identifier that avoids exposing personal names in routes.
