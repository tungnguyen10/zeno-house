...
async function openCreateRoom() {
  actionMenuOpen.value = false
  await navigateTo('/dashboard/rooms/create')
}
  ...
  <div>
    <UiPageHeader title="Phòng" :description="`${total} phòng`">
      <template #actions>
        <div class="flex items-center gap-2">
          <div
            v-if="authStore.can('rooms.create') || authStore.can('rooms.delete')"
            class="relative"
          >
            <UiButton
              variant="ghost"
            ...
            <template v-if="actionMenuOpen">
              ...
              >
                <UiButton
                  v-if="authStore.can('rooms.create')"
                  variant="ghost"
                  size="sm"
                ...
                >
                  ...
                  <span>{{ selectionMode ? 'Thoát chọn nhiều' : 'Chọn nhiều phòng' }}</span>
                </UiButton>
              </div>
            </template>
          </div>
    ...
    >
      <template v-if="authStore.can('rooms.create')" #action>
        <NuxtLink to="/dashboard/rooms/create">
