...
| GET | `/api/billing/periods/[id]/incidental-charges` |
| POST | `/api/billing/periods/[id]/incidental-charges` |
| PATCH | `/api/billing/periods/[id]/incidental-charges/[chargeId]` |
| DELETE | `/api/billing/periods/[id]/incidental-charges/[chargeId]` |
...
Bulk invoice issue is a two-request protocol. `issue-preview` accepts contract IDs and an optional `due_date_override`, then resolves a due/grace schedule per invoice from contract, building, or the system four-day fallback. It returns `calculationDate`, the override, server-rendered draft documents, exclusions, a canonical snapshot hash, and a server-owned operation ID. `issue` accepts that selection plus the returned hash and operation ID, recomputes the same date/config-aware state, rejects stale previews with `409 CONFLICT`, and passes validated server-owned draft lines with their due/grace snapshots to the atomic issue transaction. Replaying a completed operation ID returns its original invoices without modifying their schedules.

Incidental-charge create accepts `{ contract_id, label, amount, note?, operation_id }`; update and delete require `expected_updated_at`. All writes require `billing.write`, building write scope, an open period, and no non-void invoice for the target contract. CRUD writes and their billing audit events are atomic, while duplicate create retries replay by operation ID.
