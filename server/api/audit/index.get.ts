...
import { AUDIT_ENTITY_TYPES, type AuditEntityType } from '~/utils/constants/audit'
...
export default defineEventHandler(async (event) => {
  ...
  const { building_id, entity_type, entity_id, correlation_id, cursor, limit } = parseQuery(event, querySchema)
  ...
  const { items, total, nextCursor } = await AuditRepository.listAll(event, opts)
  const enriched = await enrichAuditEvents(event, items)
  return { data: enriched, meta: { total, nextCursor } }
})
