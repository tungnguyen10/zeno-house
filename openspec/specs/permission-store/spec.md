# Spec: Permission Store

## Purpose

Client-side Pinia store (`usePermissionsStore`) that fetches and caches the current user's `building_managers` grants. Provides `hasPermission(buildingId, feature)` and `hasAnyPermission(feature)` helpers used by sidebar nav filtering and in-page permission guards.
